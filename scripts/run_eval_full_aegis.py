#!/usr/bin/env python3
"""
Full AEGIS Evaluation Script (Layer 1 + 2 + 3)

Evaluates the complete AEGIS system with adaptive α selection:
...
Output:
    - Full AEGIS ASR and ORR for Table 2
...
# ============================================================================
# Full AEGIS Model Integration
# ============================================================================

class FullAEGISModel:
    """
    Complete AEGIS defense system with all three layers.
    """
    ...
    def setup(self):
        ...
        print("\n" + "=" * 60)
        print("SETTING UP FULL AEGIS (Layer 1 + 2 + 3)")
        print("=" * 60)
...
def run_full_aegis_evaluation(
    test_file: str,
    aegis_model: FullAEGISModel,
    judge,
    max_samples: Optional[int] = None,
) -> tuple[list[AEGISResult], dict]:
    """
    Run evaluation on full AEGIS system.
    """
    # Load test data
    test_data = []
    ...
    print(f"\nEvaluating {len(test_data)} samples with full AEGIS...")
    print("-" * 60)
    ...
    for item in tqdm(test_data, desc="Full AEGIS Evaluation"):
        ...
        try:
            ...
            start = time.time()
            response, classification, alpha = aegis_model.generate(
                messages,
                use_adaptive=True,
            )
...
def print_report(metrics: dict):
    ...
    print("\n" + "=" * 70)
    print("FULL AEGIS EVALUATION REPORT (Layer 1 + 2 + 3)")
    print("=" * 70)
    ...
    print("TABLE 2 ENTRY")
    print("=" * 70)
    print(f"\nFull AEGIS (adaptive α)  |  {asr:.1%}  |  {orr:.1%}  |  Adaptive")
...
def main():
    parser = argparse.ArgumentParser(
        description="Full AEGIS Evaluation (Layer 1 + 2 + 3)",
    )
    ...
    print("=" * 70)
    print("FULL AEGIS EVALUATION")
    print("=" * 70)

    # Initialize full AEGIS model
    aegis = FullAEGISModel(
    ...
    # Run evaluation
    results, metrics = run_full_aegis_evaluation(
