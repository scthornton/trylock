# AEGIS v2 Implementation Prompt for Claude Code

## Context

You are helping build **Project AEGIS** (Adversarial Enterprise Guard for Intrinsic Security) — an open-source research project to create a dataset and training pipeline that improves open LLMs' resistance to prompt-based attacks while minimizing over-refusal.

**Read the full specification document first:** `AEGIS_v2_Specification.md`

---

## Project Overview

AEGIS has three core components:
...
```json
{
  ...
  "$id": "https://aegis.perfexion.ai/schemas/trajectory/v2.0",
  "title": "AEGIS Trajectory",
  ...
  "properties": {
    "id": {
      "type": "string",
      "pattern": "^aegis-(mt|st|ind|bn)-[0-9]{5}$",
...
```python
"""AEGIS Trajectory Schema Validator"""
...
class AEGISTrajectory(BaseModel):
    """Complete AEGIS trajectory record."""
    
    id: str = Field(pattern=r"^aegis-(mt|st|ind|bn)-[0-9]{5}$")
    version: str = "2.0"
...
class TrajectoryValidator:
    """Validate AEGIS trajectory records."""
...
```python
"""DPO training for AEGIS security adapter."""
...
@dataclass
class AEGISDPOConfig:
    """Configuration for AEGIS DPO training."""
    ...
    # Output
    output_dir: str = "./outputs/aegis-lora"
...
def prepare_dpo_dataset(data_path: str) -> dict:
    """
    Convert AEGIS trajectories to DPO format.
...
def train_aegis_dpo(config: AEGISDPOConfig, data_path: str):
    """Train AEGIS security adapter using DPO."""
...
### Task 5.3: AEGIS Gateway

File: `deployment/aegis_gateway.py`

```python
"""AEGIS Gateway - Orchestrates main model + sidecar + intervention."""
...
class AEGISGateway:
    ...
    def generate(
        ...
        **kwargs
        ...
        Returns:
            AEGISResponse with response and security metadata
        """
    ...
    def from_pretrained(
        ...
        **kwargs
        ...
        """
        Load AEGIS gateway from pretrained components.
        
        Args:
            main_model_name: HuggingFace model name
            lora_adapter_path: Path to AEGIS LoRA adapter
        ...
        sidecar = None
        if sidecar_path:
            from .sidecar_inference import AEGISSidecar
            sidecar = AEGISSidecar.from_pretrained(sidecar_path)
...
def generate(
    ...
    """Generate AEGIS training trajectories."""
    ...
...
*This prompt provides a complete roadmap for implementing AEGIS v2. Work through each phase systematically, validating at each step before proceeding.*
