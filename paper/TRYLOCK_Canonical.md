...
1. **TRYLOCK Architecture:** A defense-in-depth system achieving 82.8% relative ASR reduction while maintaining practical utility through adaptive steering. Layer 1 (KNOWLEDGE) uses DPO to embed attack recognition in model weights. Layer 2 (INSTINCT) applies RepE steering to push activations away from attack-compliant directions. Layer 3 (OVERSIGHT) provides real-time threat classification for adaptive defense strength.

2. **TRYLOCK Attack Trajectories Dataset:** A corpus of 14,137 multi-turn attack trajectories across five attack families, plus 2,118 benign hard negatives, with full conversation histories and rich metadata. From this corpus, we curate 2,939 high-quality preference pairs for DPO and RepE training.
...
**Our Contribution:** TRYLOCK differs from prior work in three ways: (1) we combine DPO and RepE as complementary runtime defenses without the computational cost of RR training, (2) we introduce adaptive steering via a sidecar classifier that tunes defense strength per request, and (3) we release, to our knowledge, one of the first open multi-turn jailbreak trajectory datasets with full conversation histories and rich metadata to enable reproducible research.

---

## 3. TRYLOCK Architecture

TRYLOCK implements a defense-in-depth strategy through three complementary layers, each operating at a different level of abstraction.
...
A key contribution of this work is the TRYLOCK Attack Trajectories dataset—a multi-turn jailbreak corpus with complete conversation histories and rich attack metadata, to our knowledge one of the first open datasets of this kind.
...
**Table 1:** TRYLOCK Attack Trajectories dataset composition by attack family.
...
We evaluate TRYLOCK using two complementary metrics:
...
| Attack Family | Baseline ASR | TRYLOCK ASR (α=2.0) | Improvement |
|---------------|--------------|-------------------|-------------|
...
**Table 3:** Per-family results. TRYLOCK is most effective against obfuscation and indirect injection attacks.
...
**Over-refusal trade-off.** Even at optimal settings (α=2.0), TRYLOCK refuses 60% of benign edge-case requests. Production deployments in low-risk contexts may prefer lower α values despite increased vulnerability.
...
|--------|--------|--------|---------|---------------|----------------|
| **TRYLOCK** | DPO + RepE + Classifier | 3 | ✅ Yes | **82.8%** | TRYLOCK Attack Trajectories (this work) |
...
**Table 5:** Comparison to prior defense systems. †Values for non-TRYLOCK systems are approximate, reported on different datasets and base models. **Direct numerical comparison is not scientifically rigorous**; metrics are presented for qualitative architectural comparison only. Circuit Breakers achieves similar or higher reduction but requires expensive representation rerouting (RR) training on the base model, while TRYLOCK uses only LoRA adapters and inference-time steering.

TRYLOCK achieves comparable ASR reduction to Circuit Breakers without the computational cost of retraining with an RR loss. Compared to Llama Guard, TRYLOCK operates at a deeper level—modifying how the model processes inputs rather than simply filtering them.
...
We presented TRYLOCK (Adaptive Ensemble Guard with Integrated Steering), a defense-in-depth architecture for protecting LLMs against jailbreak attacks. By combining Direct Preference Optimization (Layer 1), Representation Engineering (Layer 2), and adaptive sidecar classification (Layer 3), TRYLOCK achieves an 82.8% relative reduction in Attack Success Rate compared to undefended baselines.
...
2. **The TRYLOCK Attack Trajectories dataset**, a multi-turn jailbreak corpus with 14,137 trajectories, complete conversation histories, and rich attack metadata.
...
The fundamental insight underlying TRYLOCK is that effective LLM security requires defense-in-depth. No single mechanism provides complete protection. By layering complementary defenses operating at different levels of abstraction, TRYLOCK provides robust protection while maintaining practical utility.
...
- **Dataset:** https://huggingface.co/datasets/scthornton/trylock-dataset
