...
**Our Contribution**: AEGIS uniquely combines all three approaches—weight modification (DPO), activation steering (RepE), and external classification (sidecar)—in a unified architecture where each layer provides independent, complementary protection.
...
AEGIS implements defense-in-depth through three complementary layers, each operating at a different level of the inference stack.
...
**Table 1**: Attack category distribution in AEGIS dataset.
...
| **AEGIS** | DPO + RepE + Classifier | **Yes** | **3** |

**Table 4**: Comparison of AEGIS to prior defense systems.
...
- **Training Dataset**: https://huggingface.co/datasets/scthornton/aegis-dataset
