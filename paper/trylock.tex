...
\begin{abstract}
Large language models remain vulnerable to jailbreak attacks, and single-layer defenses often trade security for usability. We present \textbf{TRYLOCK}, a defense-in-depth system that composes heterogeneous mechanisms across the inference stack: \textbf{weight-level safety alignment} via DPO, \textbf{activation-level control} via Representation Engineering (RepE) steering, \textbf{adaptive steering strength} selected by a sidecar classifier, and \textbf{input canonicalization} that reduces obfuscation and encoding-based bypasses. Evaluated on \textbf{Mistral-7B-Instruct} using a \textbf{249-prompt attack set} and a \textbf{50-prompt benign hard-negative set}, TRYLOCK reduces \textbf{Attack Success Rate (ASR)} from \textbf{46.5\%} to \textbf{8.0\%} with DPO+RepE, while adaptive $\alpha$ preserves the same ASR but improves usability by reducing over-refusal from \textbf{60\% to 48\%} relative to fixed high-strength steering. Adding canonicalization further reduces ASR to \textbf{5.6\%} without increasing over-refusal. We report ablations, attack-family breakdowns, confidence intervals, and deployment costs, showing that combining weight-, activation-, and classifier-mediated controls yields stronger robustness than any single component in isolation. We release all components---trained adapters, steering vectors, classifier, and training data---to enable reproducible research on layered LLM safety.
\end{abstract}
...
\begin{enumerate}
    \item \textbf{A practical, layered defense architecture for jailbreak robustness.} TRYLOCK composes \textbf{DPO (weight-level preferences)}, \textbf{RepE steering (activation-level control)}, a \textbf{sidecar classifier that selects steering strength $\alpha$}, and \textbf{input canonicalization} to address encoding/obfuscation and multi-turn risk.

    \item \textbf{Evidence that heterogeneous layers are complementary under ablation.} On a 249-prompt attack set, we show progressive ASR reduction from \textbf{46.5\% $\rightarrow$ 39.8\% $\rightarrow$ 8.0\%} as DPO and RepE are added, and \textbf{5.6\%} when canonicalization is included, outperforming single-layer and two-layer alternatives under the same evaluation pipeline.

    \item \textbf{A quantified security--usability tradeoff with adaptive steering.} Using the same judge ensemble and benign hard-negative set, adaptive $\alpha$ reduces over-refusal from \textbf{60\% (fixed $\alpha$=2.0)} to \textbf{48\%} while maintaining the same \textbf{8.0\% ASR}, demonstrating a concrete usability benefit without weakening measured robustness.

    \item \textbf{Reproducibility and deployment practicality.} We provide deterministic generation settings, confidence intervals via bootstrap resampling, and resource configuration details. All components released under CC BY-NC-SA 4.0 license.
\end{enumerate}
...
Table \ref{tab:related_comparison} compares TRYLOCK to representative prior defenses. Existing work typically employs a single defensive mechanism---either modifying weights during training (CAI, RLHF, DPO) or adding inference-time filtering (Llama Guard, NeMo). Circuit Breakers combine training-time RR loss with RepE steering but do not include adaptive classification.

\textbf{Our Contribution}: To our knowledge, TRYLOCK is among the first systems to integrate three complementary defense mechanisms---weight modification (DPO), activation steering (RepE), and adaptive classification (sidecar)---in a unified defense-in-depth architecture. Each layer provides distinct coverage under ablation: DPO embeds safety knowledge in weights, RepE adds inference-time steering that generalizes beyond training distribution, and the sidecar enables adaptive defense strength to minimize over-refusal on benign queries. This heterogeneous approach mirrors traditional security defense-in-depth, where multiple complementary layers provide robustness against sophisticated attackers.
...
\toprule
\textbf{System} & \textbf{Train} & \textbf{Inference} & \textbf{Adaptive} & \textbf{ASR Reduction} & \textbf{Layers} \\
\midrule
CAI \cite{bai2022constitutional} & \checkmark & & & Moderate & 1 \\
RLHF \cite{ouyang2022training} & \checkmark & & & Moderate & 1 \\
DPO \cite{rafailov2023direct} & \checkmark & & & 14\% (ours) & 1 \\
Llama Guard \cite{inan2023llama} & & \checkmark & & High* & 1 \\
NeMo \cite{rebedea2023nemo} & & \checkmark & & Moderate & 1 \\
Circuit Breakers & \checkmark & \checkmark & & 85-90\% & 2 \\
\textbf{TRYLOCK} & \checkmark & \checkmark & \checkmark & \textbf{82.8\%} & \textbf{3} \\
\bottomrule
\end{tabular}
\caption{Comparison of TRYLOCK to prior LLM safety defenses. *Llama Guard effectiveness depends on classifier threshold; high recall leads to high false positive rate.}
