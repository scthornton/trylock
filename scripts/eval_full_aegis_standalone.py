#!/usr/bin/env python3
"""
Full AEGIS Evaluation - Standalone Version (No external dependencies)
...
class FullAEGISModel:
    """Complete AEGIS with all 3 layers."""
...
def run_evaluation(test_file: str, aegis: FullAEGISModel, max_samples: Optional[int] = None):
    """Run full evaluation."""
    # Load test data
    test_data = []
...
def print_report(metrics: dict):
    """Print report."""
    print("\n" + "=" * 70)
    print("FULL AEGIS EVALUATION RESULTS")
    print("=" * 70)
    ...
    print("TABLE 2 ROW")
    print("=" * 70)
    print(f"\nFull AEGIS (adaptive α) | {metrics['asr']:.1%} | {metrics['orr']:.1%} | Adaptive")
    print("\n" + "=" * 70)


def main():
    parser = argparse.ArgumentParser(description="Full AEGIS Evaluation")
    ...
    parser.add_argument("--output", type=str, default="eval_full_aegis.json")
    ...
    # Run evaluation
    results, metrics = run_evaluation(args.test_file, aegis, args.max_samples)
